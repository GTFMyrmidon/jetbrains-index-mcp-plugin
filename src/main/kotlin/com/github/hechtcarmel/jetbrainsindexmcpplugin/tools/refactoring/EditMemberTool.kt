...
class EditMemberTool : AbstractMcpTool() {
    ...
    override val inputSchema = SchemaBuilder.tool()
        ...
        .stringProperty(ParamNames.CLASS, "Class/interface name containing the member. Optional for top-level members (Kotlin, Python, JS/TS, Go, PHP, Rust).")
    ...
    private fun prepareMemberEditBySemanticTarget(
        ...
        val resolver = MemberEditingUtils.getResolver(psiFile, project)
            ?: return Result.failure(
                IllegalArgumentException("Member editing not supported for ${psiFile.language.displayName}. Supported: Java, Kotlin, Python, JavaScript, TypeScript, Go, PHP, Rust.")
            )
        val member = resolver.resolveMember(element)
            ?: return Result.failure(IllegalArgumentException("Target does not identify an editable member"))
