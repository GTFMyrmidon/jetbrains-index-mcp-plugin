...
- `ide_change_signature` - Change method signature (name, return type, visibility, parameters) with automatic caller updates using IntelliJ's Change Signature refactoring. Java, Python, JS/TS. (disabled by default)
...
- `ide_replace_member` - Replace method body or field initializer only, preserving the signature (Java, Kotlin, Python, JS/TS) (disabled by default)
...
4. `ToolRegistry.registerJavaRefactoringTools()` - Registers `ide_list_tests` if Java plugin available
