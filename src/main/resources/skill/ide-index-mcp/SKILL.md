...
3. `ide_change_signature` - change method signature and update callers (disabled by default)
4. `ide_move_file` - move file and let the IDE apply semantic updates when that language/backend supports them
5. `ide_refactor_safe_delete` - preview/delete an exact symbol target or file with usage checking across languages
